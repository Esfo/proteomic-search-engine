...
class FragmentOrganizer:
    def __init__(self, librarylocation, processingdirectory, proteome, nprocs, ppmtol, ions):
        ...
        with open(linemasksbyscanbysubformulafile, 'rb') as pick:
            self.linemasksbyscanbysubformula = pickle.load(pick)
    ...
    def group_fragmentation(self, group, count):
        ...
        for subformula in groupsubformulas:
            ...
            st = time()
            for scan, linemasks in self.linemasksbyscanbysubformula[subformula].items():
                if len(linemasks) > 1:
