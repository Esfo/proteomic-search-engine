...
class FragmentOrganizer:
    def __init__(self, librarylocation, processingdirectory, fragmentlocation, proteome, nprocs, ppmtol, ions):
        self.csvfilename = fragmentlocation + 'fragments'
        ...
        for subformula in groupsubformulas:
            ...
            for scan, lines in self.linesbyscanbysubformula[subformula].items():
                ...
                for charge in range(1, maxcharge+1):
                    chargeiterations += 1
                    chargedfragments = ((fragmasses + proton * charge) / charge).tolist()
                    ...
                    for scanindex, fragindices in matches.items(): #scan index: [fragmass indices]
                        ...
                        for fragindex in fragindices:
                            ...
                            outstring = ','.join((str(i) for i in out)) + '\n' #smaller memory footprint than a list + needs to be written like this later, only ~1-2 microsecond cost
                            finaloutput.append(outstring)
            searchtime += time() - st
        fragtime = time() - nt - searchtime
        #putting this mid-loop doesn't save practically any memory
        with open(self.csvfilename + '.' + str(count) + '.matches.csv', 'w') as f:
            for piece in finaloutput:
                f.write(piece)
        print(f'{round(time() - nt, 2)} - group {count} saved, fragtime: {round(fragtime, 2)}, searchtime: {round(searchtime, 2)}, generated fragments: {fraglens}, scan masses: {scanlens}, charge-iterations: {chargeiterations}, matches: {len(finaloutput)}')
        #self.semaphore.release()
    
    def group_processing(self):
        print(len(self.dividedgroups), 'dividedgroups total')
        #processes = []
...
def fragment_writer(librarylocation, processingdirectory, fragmentlocation, proteome, nprocs, ppmtol, ions):
    
    nt = time()
    
    model = FragmentOrganizer(librarylocation, processingdirectory, fragmentlocation, proteome, nprocs, ppmtol, ions)
