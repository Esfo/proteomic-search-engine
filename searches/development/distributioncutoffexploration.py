...
for dk, lkeys in matchorganizer.items():
    ...
    for lk in lkeys.tolist(): #can this loop be made concurrent?
        ...
        if lrange > 1:
            ...
            dmax = (differencefloats.max() / lintensities.max()) / lrange
            areafloat = differencefloats.sum() / lrange
...
dsizebylrangeareafloatcutoff = maxequalmatches[:,5].max()
...
dallowancebylrangeareafloatcutoff = maxlrangedallowancearray[:,5].max()
...
allowancebylrangeareafloatcutoff = maxlrangeallowancearray[:,5].max()
...
#basically, no matter how i look at it, from either the best or the worst choices, normalized or non-normalized, i don't like the cutoff basis for either float value, i'm simply going to go with using allowance alone and rely on a 0 or something
...
n = 0
matchallowances[n] = arrayout[arrayout[:,0] == 0, 6].astype(int).tolist()
n += 1
matchallowances[n] = arrayout[arrayout[:,1] == 0, 6].astype(int).tolist()
n += 1
for acut, dcut in zip(areacutoffs, dmaxcutoffs):
    ...
    matchallowances[n] = arrayout[arrayout[:,5] < acut,6].astype(int).tolist()
    n += 1
...
for mid in random.sample(matchallowances[0], batchnumber):
