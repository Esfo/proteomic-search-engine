...
def charge_deconvolution(steplimit, newinclimit, chargetolerance, scan, maxcharge):
    precursorinfo = scan['precursorList']['precursor'][0]
    selectionwindow = precursorinfo['isolationWindow']
    precmass = selectionwindow['isolation window target m/z'].real
            ...
            if diff <= masswidthlimit:
                ...
                if charge <= maxcharge:
                    ...
                    if actualmass <= uppermasslimit:
                        ...
                        acdiff = expdiff - diff
                        diffcut = expdiff + (expdiff * chargetolerance)
...
distributionmatchfile = '/'.join((processedroot, 'distributionmatches.matches.pickle'))
...
#librarymatchfile = '/'.join((processinglocation, 'ms1.librarymatches.pickle'))
#with open(librarymatchfile, 'rb') as pick:
#    linesbylibrarymatch = pickle.load(pick)
#linesbylibrarymatch = defaultdict(list) #'librarykey + position': [lines]

#roundcutfile = '/'.join((processedroot, 'roundcutoff.pickle'))
...
analytefile = '/'.join((processedroot, 'analytefactors.pickle'))
...
distributionregionsloc = '/'.join((processedroot, 'distributionregions.pickle'))
...
regionfile = '/'.join((processedroot, 'regions.pickle'))
...
loaderloc = '/'.join((processedroot, 'trackedgroups.pickle'))
...
for output in msrun.map(lambda scan: coordinate_generation(scan), processes=nproc):
    match output:
        case tuple():
            coords, bounds = output
...
#question: does spectrabysampling need to incorporate distance from the center of the quad's target scan as a factor in quantity?
...
spectrabysampling = defaultdict(lambda: defaultdict(dict)) #scan: analyteid: coordinate from max (of distributions ie: n-1, n+1, from max): % by area of ms1 lines
...
for line, scans in scansoflines.items():
    try:
        ...
        #linecoordinate = lines.index(line)
    ...
    for scan in scans:
        ...
        spectrabysampling[scan][analyteid][linecoordinate] = sampleintensity
...
for scan, analytes in spectrabysampling.items():
    samplesum = 0
    ...
    for analyteid, positions in analytes.items():
        for position, area in positions.items():
            spectrabysampling[scan][analyteid][position] /= samplesum
    spectrabysampling[scan] = dict(analytes) #can't pickle double-default dicts
spectrabysampling = dict(spectrabysampling)

#i don't seem to use analytefragmentbounds or scanwindowbounds anywhere in any files other than this, do i need them? i'm saving them below
analytefragmentbounds = {} #analyteid: [min, max] bounds to be used for the fragment library generation
for analyte, scans in scansbyanalyte.items():
    bounds = np.array([scanwindowbounds[i] for i in scans])
    analytefragmentbounds[analyte] = [bounds[:,0].min(), bounds[:,1].max()]
...
for analyteid, samples in scanalytecharges.items():
    if analyteid in distributionmatches: #not in nodists
        ...
        for distid in unlinkedmatches.tolist(): #matches from the library
            if distid % 2:
                distid -= 1
...
print(time() - nt, 'potential sequences organized')


#~
#POTENTIAL SPEC DECONVOLUTION, WORKING
...
analytesbydistributionbyscan = {}
linesofdistributionsbyscan = {}
distributionchargesbyscan = {}
analytekeysbyscan = {}
analytedistributionsbyscan = {}

#getsize(analytesbydistributionbyscan)
#Out[13]: 234073256
#getsize(linesofdistributionsbyscan)
#Out[14]: 1222384940
#getsize(distributionchargesbyscan)
#Out[15]: 722772656
#getsize(analytekeysbyscan)
#Out[16]: 1470111584
#getsize(analytedistributionsbyscan)
#Out[17]: 1113541672
#35% memory use total
...
for scan in msrun:
    if scan['ms level'] > 1:
        index = scan['index']
        if index in maxchargeofscans:
            #chargelens[scan['index']] = charge_deconvolution_partial(scan, maxchargeofscans[scan['index']])
            try:
                #analytesbydistribution, linesofdistributions, distributioncharges, analytekeys, analytedistributions = charge_deconvolution_partial(scan, maxchargeofscans[index])
                analytedistributions = charge_deconvolution_partial(scan, maxchargeofscans[index])
                #analytesbydistributionbyscan[index] = analytesbydistribution
                #linesofdistributionsbyscan[index] = linesofdistributions
                #distributionchargesbyscan[index] = distributioncharges
                #analytekeysbyscan[index] = analytekeys
                #analytedistributionsbyscan[index] = analytedistributions
            except ValueError:
                print(index)
                pass
print(time() - nt, 'current bit')
...
#organize output to be stored
#so anything without a charge is just 1+?
#   ^i could perhaps let them be 'every' charge up to max, then phase out anything that doesn't make logical sense. i think for now i can assume it to be 1 then move forward on this as the workflow gets more sophisticated
#   ^i think this could be another factor that helps with FDR
#   ^let's find the most confident IDs from the crux runs and try both strategies
#   ^if you do an n-1 max charge, then it probably won't add any real speed problems to check them all anyways, most shit is 2+ to begin with
...
scansoflinesfile = '/'.join((processedroot, 'scansoflines.pickle'))
with open(scansoflinesfile, 'wb') as pick:
    pickle.dump(scansoflines, pick)

linesofscansfile = '/'.join((processedroot, 'linesofscans.pickle'))
with open(linesofscansfile, 'wb') as pick:
    pickle.dump(linesofscans, pick)

scansbyanalytefile = '/'.join((processedroot, 'scansbyanalyte.pickle'))
with open(scansbyanalytefile, 'wb') as pick:
    pickle.dump(scansbyanalyte, pick)

scanalytefile = '/'.join((processedroot, 'scanalytes.pickle'))
with open(scanalytefile, 'wb') as pick:
    pickle.dump(scanalytecharges, pick)

spectrafile = '/'.join((processedroot, 'spectrabysampling.pickle'))
with open(spectrafile, 'wb') as pick:
    pickle.dump(spectrabysampling, pick)

scanboundsfile = '/'.join((processedroot, 'fragmentbounds.pickle'))
with open(scanboundsfile, 'wb') as pick:
    pickle.dump(scanwindowbounds, pick)

analyteboundsfile = '/'.join((processedroot, 'analytebounds.pickle'))
with open(analyteboundsfile, 'wb') as pick:
    pickle.dump(analytefragmentbounds, pick)

pepfragsfile = '/'.join((processedroot, 'fragment.peptides.pickle'))
with open(pepfragsfile, 'wb') as pick:
    pickle.dump(generatedsequences, pick)

isotopomerpositionsfile = '/'.join((processedroot, 'isotopomersbypositions.pickle'))
with open(isotopomerpositionsfile, 'wb') as pick:
    pickle.dump(isotopomerpositionsofanalytes, pick)

spectrabyformulafile = '/'.join((processedroot, 'spectrabyformula.pickle'))
