...
distributionregionsfile = '/'.join((processinglocation, 'distributionregions.pickle'))
with open(distributionregionsfile, 'rb') as pick:
    distributionregions = pickle.load(pick)
#distributionregions = [[massmin, massmax, mintime, maxtime, # lines, area, charge, mainmass, distid]]
...
#only collect mass arrays of scans you care about
...
#different scangroups across an overlapping scangroup - yes, this is where i can get an initial estimate of entropy and what matches should make sense when overlapping distributions are present
...
#
#compare repeating spectra of lines to each other
#compare repeating spectra of dists to each other
#compare lines and distributions to scans of non-overlapped lines and dists of a ~similar mass range
#compare line and distribution spectra across charge states

#compare by counting number of total matches
#sum distance of all matches + that of those below the mean
#average match distance, both self- and inverse-weighted
#of matchable masses, sum rank intensity differences after sorting by mass

#across lines you can proton correct now by position
