...
mf = '/store/FE/mzMLs/test/190701_F350_yM2.mzML'
...
def pressurecalc(flowrate, newpercbmild):
    #input flowrate in nL/min
    columnlength = 150 #mm
    ...
    percentacetonitrile = np.asarray(newpercbmild) / 100
...
plt.plot(nx+1, newflow, '-', alpha=0.8, label='Direct Transformation')
...
plt.plot(nx+1, smooth, '-', alpha=0.8, label='LOWESS')
plt.title('Derived Flowrate')
plt.ylabel('nL/min')
plt.xlabel('Scan # (RT)')
plt.legend()
plt.show()

plt.plot(nx, newpercbmild, '-', label='new, milder')
plt.plot(nx, newpercb, '-', label='new, harsher')
plt.plot(oldmethod.index.values, oldmethod.loc[:,'percb'].values, '-', label='original')
plt.title('Derived Gradient')
plt.ylabel('%B')
plt.xlabel('Scan # (RT)')
plt.legend()
plt.show()
