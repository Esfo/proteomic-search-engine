...
folder = '/store/flowcharacterizations/round1/DDAs/'
...
for fr in files:
    ...
    gbase = ''.join(([i for i in fbase.split('_')[1] if not i.isdigit()]))
    gname = ''.join((gradientsfolder, gbase, '.method.csv'))
    try:
    ...
    gradient.rename({'Time': 'Time (m)'}, axis=1, inplace=True)
    ...
    gradient.reset_index().plot.line(x=xplotval, y='Column Volume %B', ax=ax)
    gradient.reset_index().plot.line(x=xplotval, y='Column Volumes', ax=ax)
    ...
    for pv in plottervals:
        ...
        enfs.reset_index().sort_values(by=cval, ascending=ca).plot.scatter(x=xplotval, y=pv, c='raw heights', colormap='summer', ax=ax[axval+2], alpha=0.3, norm=matplotlib.colors.LogNorm())
    ...
    enfs.plot.scatter(x='r area', y='raw heights')
    plt.xscale('log')
    plt.yscale('log')
    plt.title(ftitle)
    plt.show()
...
nl = el.max(level=0)
nl.drop(labels='Time (m)', inplace=True, axis=1)
...
results = pd.read_csv('/store/flowcharacterizations/round1/DDAs/MGFs/results.csv')
