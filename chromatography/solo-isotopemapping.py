...
for charge in chargeiterations: #ignoring 0-charge nodists here
    ...
    #chargemodel = spatial.KDTree(np.array(distmasses)[:,None])
    ...
    if moving:
        ...
        #matches1 = chargemodel.query_ball_point(np.array(oldmasses)[:,None], oldradii, workers=8).tolist()
...
sn = 0
groupsofdists = {} #dist: new chargegroup id
distributionsbychargegroup = defaultdict(set) #chargegroup id: [dist ids]
...
chargestatelines = {} #line: charge, for things in distributionsbychargegroup
chargeregions = []
...
# chargeregions:
# [mincharge, maxcharge, mintime, maxtime, number of charge states, area, most abundant chargestate, analyteid]

#chargedistkeys = chargeregions[chargeregions[:,1].argsort()][-10:,7]
chargedistkeys = chargeregions[chargeregions[:,5].argsort()][-10:,7]
