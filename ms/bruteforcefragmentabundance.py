...
eq = (c12prob*c12 + c13prob*c13)**carbons * (s32prob*s32 + s34prob*s34)**sulfurs

carboneq = (c12prob*c12 + c13prob*c13)**carbons
sulfureq = (s32prob*s32 + s34prob*s34)**sulfurs
...
for a in eq.expand().args:
    n += 1
    val = float(a.args[0])
    topes = ','.join(([str(i) for i in a.args[1:]]))
    expansions[topes] = val
expansions =  Counter(expansions)

#out = Counter()
#for comp, prob in expansions.items():
#    out[comp] = prob * 0.17498655701745272

n = 0
carbonexpansions = {}
for a in carboneq.expand().args:
    n += 1
    val = float(a.args[0])
    topes = ','.join(([str(i) for i in a.args[1:]]))
    carbonexpansions[topes] = val
carbonexpansions =  Counter(carbonexpansions)

n = 0
sulfurexpansions = {}
for a in sulfureq.expand().args:
    n += 1
    val = float(a.args[0])
    topes = ','.join(([str(i) for i in a.args[1:]]))
    sulfurexpansions[topes] = val
sulfurexpansions =  Counter(sulfurexpansions)
